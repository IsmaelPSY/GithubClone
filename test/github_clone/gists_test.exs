defmodule GithubClone.GistsTest do
  use GithubClone.DataCase

  alias GithubClone.Gists

  describe "gists" do
    alias GithubClone.Gists.Gist
  ...
  describe "saved_gists" do
    alias GithubClone.Gists.SavedGist

    import GithubClone.GistsFixtures
    import GithubClone.AccountsFixtures

    @invalid_attrs %{user_id: nil, gist_id: nil}
    ...
    test "create_saved_gist/1 with valid data creates a saved_gist" do
      ...
      assert {:ok, %SavedGist{} = saved_gist} = Gists.create_saved_gist(user, %{gist_id: gist.id})
    end
